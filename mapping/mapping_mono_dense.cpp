...
/// 从 REMODE 数据集读取数据
bool readDatasetFiles(const string& path, vector<string>& color_image_files, vector<SE3d>& poses);

/// 根据新的图像更新深度估计
bool update(const Mat& ref, const Mat& curr, const SE3d& T_C_R, Mat& depth, Mat& depth_cov);

/// 极线搜索 + NCC 匹配
bool epipolarSearch(const Mat& ref, const Mat& curr, const SE3d& T_C_R, const Vector2d& pt_ref, const double& depth_mu, const double& depth_cov, Vector2d& pt_curr);

/// 更新深度滤波器
bool updateDepthFilter(const Vector2d& pt_ref, const Vector2d& pt_curr, const SE3d& T_C_R, Mat& depth, Mat& depth_cov);
...
inline double getBilinearInterpolatedValue(const Mat& img, const Vector2d& pt) {
    ...
    return ((1 - xx) * (1 - yy) * double(d[0]) + xx * (1 - yy) * double(d[1]) +
            (1 - xx) * yy * double(d[img.step]) + xx * yy * double(d[img.step + 1])) /
           255.0;
}
...
/// test REMODE data: wget http://rpg.ifi.uzh.ch/datasets/remode_test_data.zip
int main(int argc, char** argv) {
    if (argc != 2) {
        cout << "Usage: mapping_mono_dense <path_to_remode_test_data>" << endl;
        return -1;
    }
    ...
    // 第一张图
    Mat img_ref = imread(color_image_files[0], 0);  // gray-scale image
    ...
    for (int index = 1; index < color_image_files.size(); index++) {
        ...
        Mat img_curr = imread(color_image_files[index], 0);
        if (img_curr.data == nullptr) continue;

        SE3d pose_cur_TWC = poses_TWC[index];
        SE3d pose_T_C_R = pose_cur_TWC.inverse() * pose_ref_TWC;  // 坐标转换关系： T_C_W * T_W_R = T_C_R

        update(img_ref, img_curr, pose_T_C_R, depth, depth_cov);

        imshow("depth", depth * 0.4);
        waitKey(1);

        imshow("image", img_curr);
        waitKey(1);
    }
...
bool updateDepthFilter(
    ...
    Vector2d lambdavec = Vector2d(A[3] * b(0, 0) - A[1] * b(1, 0), -A[2] * b(0, 0) + A[0] * b(1, 0)) / d;
